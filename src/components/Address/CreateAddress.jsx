...
const CreateAddress = ({
    ...
    try {
      ...
      if (id) {
        ...
        if (res.status === 200) {
          ...
          !isOrder && router.replace("/profile/address");
        } 
      } else {
  ...
  return (
    <div className="  rounded-lg   flex items-center justify-center px-2 py-10  ">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {addressFormData.map((item, idx) => (
              <div
                key={idx}
                className={`${
                  item?.className ? item?.className : "col-span-2 md:col-span-1"
                }`}
              >
                {item?.type === "select" ? (
                  <SingleSelect
